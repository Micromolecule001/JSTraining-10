...
function removeAndDouble(arr, startIdx) {
  ...
  return arr.splice(fromIndex, toIndex).map(element => element = element*2);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  return arr.flat().map(element => element*element).filter(element => element > threshold);
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо 'Вхідний параметр має бути масивом'
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const sum = lastNElements.reduce((accumulator, element) => accumulator + element, 0);
...
function expandArrayByN(arr, n) {
  ...
  return arr.flatMap(element => (element === element ? [element, element, element] : 1))
...
function findLongestWord(arr) {
  ...
  return arr.reduce((accumulator, element) => (accumulator.length < element.length ? accumulator = element : accumulator))
